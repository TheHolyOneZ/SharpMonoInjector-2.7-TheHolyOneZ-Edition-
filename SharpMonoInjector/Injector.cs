    ...
    public class Injector : IDisposable
    {
        ...
        private IntPtr Execute(IntPtr address, params IntPtr[] args)
        {
            ...
            if (Options.ObfuscateCode)
                code = ObfuscateShellcode(code);

            IntPtr alloc = _memory.AllocateAndWrite(code);

            ThreadCreationFlags creationFlags = Options.HideThreads ? ThreadCreationFlags.CREATE_SUSPENDED : ThreadCreationFlags.None;

            IntPtr thread = Native.CreateRemoteThread(_handle, IntPtr.Zero, 0, alloc, IntPtr.Zero, creationFlags, out _);
            ...
            {
                Native.NtSetInformationThread(thread, 0x11, IntPtr.Zero, 0);
        ...
        private byte[] ObfuscateShellcode(byte[] code)
        {
            byte xorKey = (byte)_random.Next(1, 256);
            
            IntPtr encryptedAddr = _memory.AllocateAndWrite(new byte[code.Length]);
            
            for (int i = 0; i < code.Length; i++)
            {
                byte[] encryptedByte = new byte[] { (byte)(code[i] ^ xorKey) };
                Native.WriteProcessMemory(_handle, encryptedAddr + i, encryptedByte, 1);
            }

            List<byte> decoder = new List<byte>();
            
            if (Is64Bit)
            {
                decoder.AddRange(new byte[] { 0x48, 0xB8 });
                decoder.AddRange(BitConverter.GetBytes((long)encryptedAddr));
                decoder.AddRange(new byte[] { 0x48, 0xB9 });
                decoder.AddRange(BitConverter.GetBytes((long)code.Length));
                decoder.AddRange(new byte[] { 0x48, 0x31, 0xD2 });
                decoder.AddRange(new byte[] { 0xB3, xorKey });
                decoder.AddRange(new byte[] { 0x8A, 0x04, 0x10 });
                decoder.AddRange(new byte[] { 0x30, 0xD8 });
                decoder.AddRange(new byte[] { 0x88, 0x04, 0x10 });
                decoder.AddRange(new byte[] { 0x48, 0xFF, 0xC2 });
                decoder.AddRange(new byte[] { 0x48, 0x39, 0xCA });
                decoder.AddRange(new byte[] { 0x75, 0xF1 });
                decoder.AddRange(new byte[] { 0x48, 0xB8 });
                decoder.AddRange(BitConverter.GetBytes((long)encryptedAddr));
                decoder.AddRange(new byte[] { 0xFF, 0xE0 });
            }
            else
            {
                decoder.AddRange(new byte[] { 0xB8 });
                decoder.AddRange(BitConverter.GetBytes(encryptedAddr.ToInt32()));
                decoder.AddRange(new byte[] { 0xB9 });
                decoder.AddRange(BitConverter.GetBytes(code.Length));
                decoder.AddRange(new byte[] { 0x31, 0xD2 });
                decoder.AddRange(new byte[] { 0xB3, xorKey });
                decoder.AddRange(new byte[] { 0x8A, 0x04, 0x10 });
                decoder.AddRange(new byte[] { 0x30, 0xD8 });
                decoder.AddRange(new byte[] { 0x88, 0x04, 0x10 });
                decoder.AddRange(new byte[] { 0x42 });
                decoder.AddRange(new byte[] { 0x39, 0xCA });
                decoder.AddRange(new byte[] { 0x75, 0xF6 });
                decoder.AddRange(new byte[] { 0xB8 });
                decoder.AddRange(BitConverter.GetBytes(encryptedAddr.ToInt32()));
                decoder.AddRange(new byte[] { 0xFF, 0xE0 });
            }
